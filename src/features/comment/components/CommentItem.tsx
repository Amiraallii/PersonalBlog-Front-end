...
interface CommentItemProps {
  ...
  onActionStart: () => void;
  onActionEnd: () => void;
  onShowRepliesClick?: (commentId: string) => void;
}
...
const CommentItem = ({
  ...
  return (
    <div className="relative w-full bg-[var(--surface)] mb-2 rounded-xl border border-[var(--border)] p-3 transition-colors select-none">
      <div
        {...longPressHooks}
        className="flex gap-3 active:bg-[rgba(255,255,255,0.02)] rounded-lg p-1 transition-colors cursor-pointer"
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-[var(--text)] leading-relaxed whitespace-pre-line break-words">
            {comment.content}
          </p>
      ...
      {replyCountNumber > 0 && (
        <div className="flex items-center gap-3 mt-3 mr-12">
          ...
          >
            {areRepliesVisible
              ? "پنهان کردن پاسخ‌ها"
              : `مشاهده همه پاسخ‌ها (${comment.replyCount})`}
          </button>
        </div>
      )}
      ...
      {showActionMenu && (
        <div
          className="fixed inset-0 z-[2000] bg-black/60 backdrop-blur-sm flex items-end justify-center p-4 animate-fade-in"
          onClick={() => setShowActionMenu(false)}
        >
          <div
            className="w-full max-w-sm bg-[var(--surface)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-2xl p-2 animate-slide-up"
