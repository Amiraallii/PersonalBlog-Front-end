import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import swal from "sweetalert";
import {
  PencilSquareIcon,
  PhoneIcon,
  EnvelopeIcon,
  GlobeAltIcon,
  PencilIcon,
} from "@heroicons/react/16/solid";

import { useAuth } from "../../../context/AuthContext";
import { ROLES } from "../../../types/auth";
import { AboutMeService } from "../services";
import type { PersonalInfo } from "../types";
import { LocationPicker, parseLocation } from "../../../components/Location";

const AboutMeIndex = () => {
  const [personalInfo, setPersonalInfo] = useState<PersonalInfo | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const navigate = useNavigate();
  const { role } = useAuth();

  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const data = await AboutMeService.get();
        setPersonalInfo(data);
      } catch (error) {
        console.error(error);
        swal("خطا", "ارتباط با سرور برقرار نشد!", "error");
      } finally {
        setIsLoading(false);
      }
    };
    fetchData();
  }, []);

  const getContactIcon = (type: number) => {
    const iconClass = "w-5 h-5";
    switch (type) {
      case 0:
        return <PhoneIcon className={iconClass} />;
      case 1:
        return <EnvelopeIcon className={iconClass} />;
      case 2:
        return (
          <svg className={iconClass} fill="currentColor" viewBox="0 0 24 24">
            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
          </svg>
        );
      case 3:
        return (
          <svg className={iconClass} fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.161c-.18 1.897-.962 6.502-1.359 8.627-.168.9-.5 1.201-.82 1.23-.697.064-1.226-.461-1.901-.903-1.056-.692-1.653-1.123-2.678-1.799-1.185-.781-.417-1.21.258-1.911.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.139-5.062 3.345-.479.329-.913.489-1.302.481-.428-.008-1.252-.241-1.865-.44-.752-.244-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.831-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635.099-.002.321.023.465.14.121.099.155.232.171.326.016.093.036.306.02.472z" />
          </svg>
        );
      case 4:
        return (
          <svg className={iconClass} fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
          </svg>
        );
      case 5:
        return (
          <svg className={iconClass} fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678c-3.405 0-6.162 2.76-6.162 6.162 0 3.405 2.76 6.162 6.162 6.162 3.405 0 6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 0-1.44-.646-1.44-1.44 0-.794.646-1.439 1.44-1.439.793-.001 1.44.645 1.44 1.439z" />
          </svg>
        );
      case 6:
        return (
          <svg className={iconClass} fill="currentColor" viewBox="0 0 24 24">
            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
          </svg>
        );
      default:
        return <GlobeAltIcon className={iconClass} />;
    }
  };

  const getContactLink = (type: number, value: string) => {
    switch (type) {
      case 0:
        return `tel:${value}`;
      case 1:
        return `mailto:${value}`;
      case 2:
        return value.startsWith("http")
          ? value
          : `https://linkedin.com/in/${value}`;
      case 3:
        return value.startsWith("http") ? value : `https://t.me/${value}`;
      case 4:
        return `https://wa.me/${value.replace(/\D/g, "")}`;
      case 5:
        return value.startsWith("http")
          ? value
          : `https://instagram.com/${value}`;
      case 6:
        return value.startsWith("http")
          ? value
          : `https://twitter.com/${value}`;
      default:
        return value.startsWith("http") ? value : `https://${value}`;
    }
  };

  return (
    <>
      <div className="w-full flex flex-col gap-4">
        {isLoading ? (
          <div className="flex justify-center items-center py-20">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--accent)]"></div>
          </div>
        ) : !personalInfo ? (
          <div className="w-full bg-[var(--surface)] rounded-xl border border-[var(--accent)] p-10 flex flex-col items-center justify-center">
            <div className="w-16 h-16 rounded-full bg-[rgba(255,255,255,0.05)] flex items-center justify-center mb-4">
              <PencilSquareIcon className="w-8 h-8 text-[var(--muted)]" />
            </div>
            <p className="text-[var(--muted)] mb-4">
              هنوز هیچ محتوایی اضافه نشده است
            </p>
            {role === ROLES.ADMIN && (
              <button
                onClick={() => navigate("/AboutMe/Modify")}
                className="px-6 py-2 bg-[var(--accent)] text-white rounded-lg hover:opacity-90 transition-opacity"
              >
                افزودن اطلاعات
              </button>
            )}
          </div>
        ) : (
          <div className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden hover:border-[var(--accent)] transition-all duration-300 shadow-lg">
            <div className="bg-gradient-to-r from-[var(--accent)] to-[var(--accent-hover)] p-6 relative">
              {role === ROLES.ADMIN && (
                <button
                  onClick={() => navigate("/AboutMe/Modify")}
                  className="absolute top-4 left-4 p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors backdrop-blur-sm"
                  title="ویرایش"
                >
                  <PencilIcon className="w-5 h-5 text-white" />
                </button>
              )}
              <div className="flex items-center gap-4">
                <div>
                  <h2 className="font-bold text-2xl text-white mb-1">
                    {personalInfo.name} {personalInfo.lastName}
                  </h2>
                  <p className="text-white/90 text-lg">
                    {personalInfo.jobTitle}
                  </p>
                </div>
              </div>
            </div>

            <div className="p-6">
              <h3 className="font-bold text-lg text-[var(--text)] mb-3 flex items-center gap-2">
                <span className="w-1 h-6 bg-[var(--accent)] rounded-full"></span>
                درباره من
              </h3>
              <p className="text-[var(--muted)] leading-relaxed">
                {personalInfo.aboutMe}
              </p>
            </div>

            {personalInfo.contactInfo &&
              personalInfo.contactInfo.length > 0 && (
                <div className="p-6 pt-0">
                  <h3 className="font-bold text-lg text-[var(--text)] mb-4 flex items-center gap-2">
                    <span className="w-1 h-6 bg-[var(--accent)] rounded-full"></span>
                    راه‌های ارتباطی
                  </h3>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {personalInfo.contactInfo.map((contact, index) =>
                      contact.contactWayType === 8 ? null : (
                        <a
                          key={index}
                          href={getContactLink(
                            contact.contactWayType,
                            contact.contactWay,
                          )}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-3 p-3 bg-[var(--background)] rounded-lg border border-[var(--border)] hover:border-[var(--accent)] hover:bg-[var(--surface)] transition-all duration-200 group"
                        >
                          <div className="w-10 h-10 rounded-full bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)] group-hover:bg-[var(--accent)] group-hover:text-white transition-colors">
                            {getContactIcon(contact.contactWayType)}
                          </div>
                          <span className="text-[var(--text)] group-hover:text-[var(--accent)] transition-colors flex-1 truncate">
                            {contact.contactWay}
                          </span>
                        </a>
                      ),
                    )}
                  </div>

                  {(() => {
                    const locationContact = personalInfo.contactInfo.find(
                      (c) => c.contactWayType === 8,
                    );
                    if (!locationContact) return null;

                    const parsed = parseLocation(locationContact.contactWay);
                    if (!parsed) return null;

                    const { lat, lng } = parsed;

                    return (
                      <div
                        className="mt-3 rounded-xl overflow-hidden border border-[var(--border)]"
                      >
                        <LocationPicker
                          value={locationContact.contactWay}
                          readOnly={true}
                          defaultCenter={[lat, lng]}
                          defaultZoom={12}
                        />
                        <div className="p-2 bg-[var(--surface)] flex items-center justify-between">
                          <a
                            href={`https://www.google.com/maps/search/?api=1&query=${lat},${lng}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-xs text-[var(--accent)] hover:underline"
                          >
                            باز کردن در Google Maps ↗
                          </a>
                        </div>
                      </div>
                    );
                  })()}
                </div>
              )}
          </div>
        )}
      </div>
    </>
  );
};

export default AboutMeIndex;
