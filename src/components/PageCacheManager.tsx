...
interface PageCacheManagerProps<T> {
  cacheKey: string;
  children: (cache: {
    cachedItems: T[];
    cachedSkip: number;
...
export default function PageCacheManager<T>({
  cacheKey,
  children,
}: PageCacheManagerProps<T>) {
  const getValidCache = (): CacheData<T> | null => {
    const saved = sessionStorage.getItem(`page_cache_${cacheKey}`);

    if (!saved) return null;

    try {
      const parsed: CacheData<T> = JSON.parse(saved);
